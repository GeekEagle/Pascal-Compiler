...
SYMLIST* listAddSym(SYMLIST* list, _SYMBOL sym);
int SYMINLIST(_SYMBOL sym, SYMLIST* list);
...
void WriteObjCode(char *filename)  //将产生的代码写进*.pld文件里面（二进制形式）
{
	...
	errno_t ero = fopen_s(&fcode,filename,"wb");
	if(ero!=0)
		error(40);  //不能打开.pld文件
	for(int i=0;i<CX;i++)
	{
...
void WriteCodeList(char *filename)  //将产生的代码写进*.lst文件（可见字符形式）
{
	...
	errno_t err  =fopen_s(&flist,filename,"wb");
	if(err!=0)
		error(39);  //不能打开.lst文件
	for(int i=0;i<CX;i++)
	{
...
void WriteLabelCode(char *filename)  //将产生的代码写进*.lab文件（可见字符形式）
{
	...
	errno_t fla = fopen_s(&flabel,filename,"wb");
	if(fla!=0)
		error(41);  //不能打开.lst文件
	for(int i=0;i<JX;i++)
	{
...
{						//如果在同一层次内有两次定义性质的出现，则报错
	...
	{
		strcpy_s(NAMETAB[0].name,CurSymbol->value.lpValue);    //当前标识符（注意：只能是标识符）的内容填入符号表的第一项？？？？？？？？？？
		j=BTAB[DISPLAY[displayLevel]].last;   //将临时变量j设成当前DISPLAY表项所指向的程序体表项的last域
		l=j;
		while(_stricmp(NAMETAB[j].name,CurSymbol->value.lpValue))  
		...
		{             //否则，填入相应的信息
			TX++;
			strcpy_s(NAMETAB[TX].name,CurSymbol->value.lpValue);
			NAMETAB[TX].link=l;
...
int GETPOSITION(char * id)  //通过标识符的内容在名字表里面查找其索引
{
	...
	strcpy_s(NAMETAB[0].name,id);
	//j=displayLevel;

	do              //在所有的活动记录（注意：是活动的）里面顺藤摸瓜
	{
		i=BTAB[DISPLAY[j]].last;
		while (_stricmp(NAMETAB[i].name,CurSymbol->value.lpValue))
			i=NAMETAB[i].link;
...
void PROCDECLARATION(SYMLIST * list)  //过程声明
{
	...
	{
		error(14);  //应该是标识符
		strcpy_s(CurSymbol->value.lpValue,strlen("")+1, "");
	}
...
void ASSIGNMENT(SYMLIST * list)  //分析赋值语句
{
	...
	i=GETPOSITION(CurSymbol->value.lpValue);    //找到变量
	if(i==0)
		error(33);   //标识符没有定义
	else
	{
...
void REPEATSTATEMENT(SYMLIST* list)  //repeat 语句的分析
{
	...
	JUMADRTAB[JX] = CX;
	JX++;
	jumpback = CX;  //纪录repeat循环执行时需要往前返回的地址
	STATEMENT(list);
	/////////////////////////////////////////////////////////////////
//	SYMLIST* tempList = new SYMLIST;
...
//	delete tempList;
	...
	/////////////////////////////////////////////////////////////////
	SYMLIST* tempList1 = new SYMLIST;
	COPYLIST(tempList1, listAddSym(list, DOSYM));
...
void CASESTATEMENT(SYMLIST* list)
{
	...
	do
	{
		...
		getASymbol();
		if (CurSymbol->type == OTHERWISESYM) {
			break;
		}
		SYMLIST* tempList1 = new SYMLIST;
		COPYLIST(tempList1, listAddSym(list, SEMICOLON));
	...
	if(CurSymbol->type==IDENT)
	{
		...
		if(NAMETAB[i].kind==PROCEDURE)
		{
			...
			else
			{
				...
				if(cp<lastPar)
					error(30);//实在参数个数不够
				GEN(CALS,NAMETAB[i].level,NAMETAB[i].unite.address);
...
SYMBOL GetReserveWord(char *nameValue)//判断得到的符号是否是保留字
{
	...
	char reserveWord[NUMOFWORD][20]=
	{
		...
		"array","call","do","end","mod","of","procedure","then","var","for","repeat",
		"until","to","case"
	};
	SYMBOL reserveType[NUMOFWORD]=
	{
		ANDSYM,BEGINSYM,CONSTSYM,ELSESYM,IFSYM,NOTSYM,ORSYM,PROGRAMSYM,TYPESYM,WHILESYM,
		ARRAYSYM,CALLSYM,DOSYM,ENDSYM,MODSYM,OFSYM,PROCSYM,THENSYM,VARSYM,FORSYM,REPEATSYM,
		UNTILSYM,TOSYM,CASESYM
	};

	for(i=0;i<NUMOFWORD;i++)  //这里采用了遍历的做法，但是效率不高，可以考虑采用二叉查找法
		if(!_stricmp(reserveWord[i],nameValue))
...
void getSymbols(FILE *srcFile)  //从源文件读入字符，获得符号链表
{
	...
	while(!Feof(srcFile))   //只要文件不空
	{
		readChar=fgetc(srcFile);  //读一个字节

		if(iscsymf(readChar))  //找出字母和下划线
		{
			nameValueint=0;
			do
			{
			...
			if(!(current->type=GetReserveWord(nameValue)))
			{
				...
				strcpy_s(current->value.lpValue,strlen(nameValue)+1,nameValue);
			}
			current->next=NULL; 
		}
...
int main(int argc, char* argv[])  
{
	...
	int len = strlen("C:\\Users\\10334\\Desktop\\");
	strcpy_s(srcFilename,len+1,"C:\\Users\\10334\\Desktop\\");
	FILE *srcFile;
	char *srcFileNamePoint;
	char name[50];
	if(argc>1)
		strcpy_s(srcFilename,argv[1]);
	else
	{
		printf("Please input the source file name : ");
		scanf_s("%s",name,strlen(name));
	}
	int length = strlen(srcFilename) + strlen(name);
	strcat_s(srcFilename,length+1,name);
	errno_t srcf = fopen_s(&srcFile, srcFilename, "r");
	if (srcf != 0)
	{
	...
	if(nError==0)
	{
	    ...
	    *srcFileNamePoint=0;  //删除后面的扩展名
		int numa = strlen(srcFilename)+strlen(".lst");
		strcat_s(srcFilename, numa+1, ".lst");
		WriteCodeList(srcFilename);
		for(srcFileNamePoint=&srcFilename[strlen(srcFilename)];*srcFileNamePoint!='.' && srcFileNamePoint!=srcFilename;srcFileNamePoint--)
			;
		*srcFileNamePoint=0;  //删除后面的扩展名
		int numb = strlen(srcFilename) + strlen(".pld");
		strcat_s(srcFilename, numb+1, ".pld");
		WriteObjCode(srcFilename);
		for(srcFileNamePoint=&srcFilename[strlen(srcFilename)];*srcFileNamePoint!='.' && srcFileNamePoint!=srcFilename;srcFileNamePoint--)
			;
		*srcFileNamePoint=0;  //删除后面的扩展名
		int numc = strlen(srcFilename) + strlen(".lab");
		strcat_s(srcFilename,numc+1,".lab");
		WriteLabelCode(srcFilename);
		destroySymbols();
		fclose(srcFile);
